...
const Chip: React.FC<{ id: string; x: number; y: number; fade: number; scale: number }> = ({ id, x, y, fade, scale }) => {
  ...
  return (
    <WorldPoint x={x} y={y} style={{ display: "flex", alignItems: "center", gap: 14, opacity: fade, transform: `translate(-50%, -50%) scale(${scale})` }}>
...
export const Scene10Closing: React.FC = () => {
  ...
  return (
    <SceneFade duration={DUR} inFrames={20} outFrames={70}>
      <AbsoluteFill>
        <SceneBackdrop ambientOpacity={0.5} />
        ...
        <AbsoluteFill style={{ transform: worldCameraTransform(cam) }}>
          ...
          <WorldPoint x={0} y={HEADER_Y} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
